class Chat{
  ...
  preload(){
    ...
    this.game.level.socket.on('playerConnected', function(p) {
      chat.open_small('L\'Usine ' + p.name + ' vient de s\'installer en ville!');
      chat.game.time.setTime(p.initialDay);
    }, this);
    this.game.level.socket.on('playerDisconnected', function(p) { chat.open_small('L\'Usine ' + p.name + ' vient de quitter la ville!') }, this);
